    ...
    def __init__(
        ...
        Args:
            host: Database host (Aurora cluster endpoint or RDS instance endpoint). Used to
                sign the IAM auth token and as the connection-map identity; must be the real
                endpoint even when the socket is opened elsewhere (see connect_host).
            port: Database port. Used for IAM token signing (the port RDS listens on); must be
                the real DB port (typically 5432) even when connect_port differs.
            ...
            connect_host: Host to actually open the TCP socket to. Defaults to host. Set this
                (e.g. 127.0.0.1) when reaching the DB through an SSH tunnel/proxy while the IAM
                token stays signed for the real endpoint (host).
        ...
        # tunnel/proxy. The IAM token (get_iam_auth_token) is still signed for self.host/self.port.
